...
##            g.msgbox('ID: '+rk+"\n"+'信息: '+information+"\n"+'来源: '\
##                     +fieldValues[0]+"\n"+'数量: '+fieldValues[1]+"\n"+'库位: '+kuwei+"\n"+'时间: '+now()+"\n"+'原库存'\
##                     +str(int(table0.row_values(i0)[4]))+'+'+'变化量'+fieldValues[1]+'='\
##                     +'最新库存'+str(int(table0.row_values(i0)[4])+int(fieldValues[1]))+"\n"+'录入人员:'+getpass.getuser(),"入库成功")
...
def generagteBarCode():
    ...
    draw.text((imgSize[0]*0.05, (imgSize[1]*0.7)), comments, (0,0,0),font=font)
...
def menu():
    ...
    #image='python.png'
    #g.msgbox(msg,title)
    choices=['搜索','信息','入库','出库','新增','移库','库存','历史','导出','预警','盘点','条码','退出']
...
#print ('入库')
    ...
    if mode==0:
        ...
        try:
            ...
            g.msgbox('ID: '+rk+"\n"+'信息: '+information+"\n"+'来源: '\
                     +fieldValues[0]+"\n"+'数量: '+fieldValues[1]+"\n"+'库位: '+fieldValues[2]+"\n"+'时间: '+now()+"\n"+'原库存'\
                     +str(int(table0.row_values(i0)[4]))+'+'+'变化量'+fieldValues[1]+'='+'最新库存'+str(int(table0.row_values(i0)[4])\
